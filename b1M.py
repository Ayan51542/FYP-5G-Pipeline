...
def split_optional_sensing_header(payload):
    if len(payload) < 8:
        return None, payload
    try:
        sensing_energy = struct.unpack(">d", payload[:8])[0]
    except struct.error:
        return None, payload
...
def process_incoming_frame(src_id, dst_id, encoded_bytes, hop_count=0):
    ...
    try:
        ...
        prediction = ML_RUNTIME.score_packet(
        ...
        if prediction is not None:
            ...
            if prediction.is_threat:
                print(
                    f"[BS {BS_ID}] [ML-THREAT] {src_id}->{dst_id} "
                    f"(RF={prediction.rf_probability:.3f}, XGB={prediction.xgb_probability:.3f}, "
                    f"Ensemble={prediction.ensemble_probability:.3f})"
                )
                if ML_ENFORCE_BLOCKING and src_id.upper() not in PRIMARY_SENDERS:
                    print(f"[BS {BS_ID}] [ML-THREAT] Blocking packet from {src_id}.")
                    with stats_lock:
                        stats["ml_blocked"] += 1
                    return
...
def export_base_station_analysis():
    ...
    if threat_history:
        fig = plt.figure(figsize=(12, 5))
        t0 = threat_history[0][0]
        times = [entry[0] - t0 for entry in threat_history]
        probs = [entry[4] for entry in threat_history]
        colors = ["red" if p >= ML_THREAT_THRESHOLD else "green" for p in probs]
        plt.plot(times, probs, color="black", linewidth=1.0, alpha=0.6)
        plt.scatter(times, probs, c=colors, s=45, alpha=0.8, edgecolor="black", linewidth=0.5)
        plt.axhline(ML_THREAT_THRESHOLD, color="red", linestyle="--", linewidth=1.2, label=f"Threat threshold ({ML_THREAT_THRESHOLD:.2f})")
        plt.ylim(0.0, 1.0)
        plt.title(f"[BS {BS_ID}] ML Threat Probability Timeline")
        plt.xlabel("Time (s)")
    ...
    try:
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as sock:
            ...
            while not STOP.is_set():
                ...
                if first_byte == b'{':
                    threading.Thread(
                        target=handle_registered_client,
                        args=(conn, addr),
                        daemon=True
                    ).start()
                else:
                    threading.Thread(
                        target=handle_hop_connection,
                        args=(conn, addr),
                        daemon=True
                    ).start()
